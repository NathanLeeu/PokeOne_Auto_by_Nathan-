...
FIGHT_IMAGE_PATH = r"D:\PokeOne\PokeBot\Bot\Image\Fight.png"
...
def extract_text_around_center(screen, center_pos):
    ...
    for region in skill_regions:
        ...
        skill_texts.append((text, region))
    
    return skill_texts

# Xác định số lượng PP còn lại của kỹ năng
def extract_pp(screen, center_pos):
    print("Đang trích xuất số lượng PP còn lại...")
    pp_regions = [
    ...
    pp_texts = []
    for region in pp_regions:
        x1, y1, x2, y2 = region
        x1, y1 = max(0, x1), max(0, y1)
        x2, y2 = min(screen.shape[1], x2), min(screen.shape[0], y2)
        pp_img = screen[y1:y2, x1:x2]
        preprocessed_img = preprocess_image(pp_img)
        text = pytesseract.image_to_string(preprocessed_img, config='--psm 7').strip()
        pp_texts.append(text)
...
def find_highest_power_skill(skill_texts, pp_texts):
    skill_powers = []
    for i, (text, region) in enumerate(skill_texts):
        if text:
            skill_info = find_skill_info(text)
            if skill_info and 'power' in skill_info:
                # Cập nhật thông tin về PP còn lại
                pp_left = pp_texts[i] if i < len(pp_texts) else ""
                skill_powers.append((skill_info['name'], skill_info['power'], region, pp_left))
    if skill_powers:
        # Chọn kỹ năng có power cao nhất và PP còn lại nhiều nhất
        return max(skill_powers, key=lambda x: (x[1] if x[1] is not None else 0, int(x[3]) if x[3].isdigit() else 0))
    return None
...
def click_randomly():
    print("Nhấp chuột ngẫu nhiên trên màn hình...")
    screen_width = 1920
    screen_height = 1080
    random_x = random.randint(0, screen_width)
    random_y = random.randint(0, screen_height)
    click_position(random_x, random_y)
            ...
            if not switch_to_pokeone_window():
                ...
                if mode == 'AFK':
                    ...
                    if center_pos:
                        ...
                        pp_texts = extract_pp(screen, center_pos)
                        print("Tên các kỹ năng được phát hiện:")
                        for text, region in skill_texts:
                            if text:
                                skill_info = find_skill_info(text)
                                if skill_info:
                        ...
                        # Chọn kỹ năng có power cao nhất và PP còn lại nhiều nhất
                        highest_power_skill = find_highest_power_skill(skill_texts, pp_texts)
                        if highest_power_skill:
                            print(f"Kỹ năng có power cao nhất: {highest_power_skill[0]} với power {highest_power_skill[1]} và PP còn lại {highest_power_skill[3]}")
                            # Nhấp vào vị trí trung tâm của vùng tọa độ tương ứng với kỹ năng này
                            x1, y1, x2, y2 = highest_power_skill[2]
                            center_x = (x1 + x2) // 2
                            center_y = (y1 + y2) // 2
                            click_position(center_x, center_y)
                        else:
                    ...
                    if not check_and_click_button(FIGHT_IMAGE_PATH):
                        for _ in range(random.randint(2, 8)):
                            # Di chuyển qua trái
                            windll.user32.keybd_event(0x25, 0, 0, 0)  # VK_LEFT
                            time.sleep(0.1)
                            windll.user32.keybd_event(0x25, 0, 2, 0)  # VK_LEFT
                        for _ in range(random.randint(2, 8)):
                            # Di chuyển qua phải
                            windll.user32.keybd_event(0x27, 0, 0, 0)  # VK_RIGHT
                            time.sleep(0.1)
                            windll.user32.keybd_event(0x27, 0, 2, 0)  # VK_RIGHT
        except Exception as e:
            print(f"Đã xảy ra lỗi: {e}")
        time.sleep(5)

if __name__ == "__main__":
    mode = input("Chọn chế độ (AFK/EXP): ").strip().upper()
    main_loop(mode)
